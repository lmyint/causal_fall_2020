...
Simulate a chain `X -> Y -> Z` where all three variables are quantitative.
...
Simulate a collider `X -> Y <- Z` where `Y` also has a child `A` (`Y -> A`). Let `X` and `A` be quantitative, and let `Y` and `Z` be binary.

Use appropriate check(s) to verify the marginal and conditional relations in this structure.
